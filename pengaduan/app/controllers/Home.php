		...
		if ($result) {
			foreach ($result as $row) {
				...
				if ($level == 'admin') {
					...
					$results = $this->model('Home_model')->getPengaduanNIK($nik);
					foreach($results as $pengaduans){
						$id_peg = $pengaduans['id_laporan'];
					}
					$this->model('Home_model')->deleteUser($id);
					$this->model('Home_model')->deletePengaduan($nik);
					$this->model('Home_model')->deleteTanggapan($id_peg);
