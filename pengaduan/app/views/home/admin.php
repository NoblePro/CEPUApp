...
<head>
	...
	<script type="text/javascript">
		function tekan() {
			...
			$("#logout").hide();
			$("#user").hide();
			$("textarea").attr("style", "height:639px !important;");
			window.print();
			$("textarea").attr("style", "height:100% !important;");
			$("#print").show();
			$("#logout").show();
			$("#user").show();
		}
...
<body>
	<center>
		...
		<div style="margin-bottom:60px; margin-top:30px">
			<a href="<?= BASEURL ?>login" class="btn btn-outline-info" id="logout">LOGOUT</a>
			<a href="<?= BASEURL ?>home/users" class="btn btn-outline-info" id="user">USERS</a>
		</div>
		<div style="margin-bottom:60px; margin-top:-50px">
			<button class="btn btn-outline-info" id="print" onclick="tekan()">PRINT</button>
		</div>
	</center>
</body>
